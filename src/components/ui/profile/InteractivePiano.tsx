...
import { FaPlay, FaPause, FaHeadphones, FaCompactDisc } from "react-icons/fa";
import { scaleIn } from "./constants";
import { getRandomMelody, type MelodyNote, type Melody } from "./melodies";
...
function InteractivePiano() {
	...
	const isPianoInView = useInView(pianoContainerRef, { amount: 0.3 });
	...
	const activeAudioNodesRef = useRef<Map<number, { stop: () => void }>>(new Map());
	...
	const keyboardShortcuts = useMemo(() => {
		// Désactiver les raccourcis clavier si le piano n'est pas visible
		if (!isPianoInView) return undefined;
	...
	}, [firstNote, lastNote, isPianoInView]);
	...
	const playNote = (midiNumber: number) => {
		...
		if (instrument) {
			...
			const audioNode = instrument.play(noteName);
			activeAudioNodesRef.current.set(midiNumber, audioNode);
		}
		setActiveNotes(prev => [...prev, midiNumber]);
	};

	const stopNote = (midiNumber: number) => {
		// Arrêter proprement l'audio
		const audioNode = activeAudioNodesRef.current.get(midiNumber);
		if (audioNode) {
			audioNode.stop();
			activeAudioNodesRef.current.delete(midiNumber);
		}
	...
	const playMelody = useCallback(async () => {
		...
		const endTimeout = setTimeout(() => {
			...
			setIsPlaying(false);
			setActiveNotes([]);
			// Nettoyer toutes les notes audio restantes
			activeAudioNodesRef.current.forEach(node => node.stop());
	...
	const stopMelody = useCallback(() => {
		...
		timeoutsRef.current.forEach(clearTimeout);
		timeoutsRef.current = [];
		// Arrêter toutes les notes audio en cours
		activeAudioNodesRef.current.forEach(node => node.stop());
		activeAudioNodesRef.current.clear();
	}, []);

	// Arrêter la mélodie si le piano sort de la vue
	useEffect(() => {
		if (!isPianoInView && isPlaying) {
			stopMelody();
		}
	}, [isPianoInView, isPlaying, stopMelody]);
		...
		>
			...
			<div className="space-y-2 text-left">
				...
				<p className="text-base text-neutral-400 leading-relaxed">
					Aujourd'hui, c'est le <span className="text-[#c8f560]">code</span> que je compose, mais
					j'ai gardé le tempo.
				</p>
			</div>
			...
			<div className="bg-neutral-900 p-5 rounded-2xl shadow-2xl border border-neutral-800 relative w-full">
				...
				{isPlaying && currentMelody && (
					<motion.div
						...
						exit={{ opacity: 0, y: -10 }}
						className="mb-4 flex items-center gap-3 bg-neutral-800/50 px-4 py-2 rounded-lg border border-neutral-700"
					>
						<motion.div
							animate={{ rotate: 360 }}
							transition={{
						...
						>
							<FaCompactDisc className="text-[#c8f560] text-xl" />
						</motion.div>
						<div className="flex-1">
							<p className="text-xs text-neutral-500 uppercase tracking-wider mb-0.5">En cours</p>
							<p className="text-sm font-medium text-neutral-200">{currentMelody.name.slice(3)}</p>
						</div>
					</motion.div>
				)}
